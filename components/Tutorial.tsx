...
function buildSteps(): Record<TutorialRole, Step[]> {
  ...
  return {
    ...
    customers: [
      ...
      criteriaStep,
      { t: 'One score per joke', b: `Criteria that run on a scale score 1 for an exact match, 0.5 one step away, 0 beyond — being close only half counts, and two steps off scores the same as being completely wrong. The four categorical criteria are all-or-nothing, and Title Fit grades itself on its own scale. Add them up for the joke's true fit, from 0 to ${MAX_FIT}. Clear a customer's bar (around τ = ${CFG.tau}) and they buy.`, v: () => <FitBar /> },
